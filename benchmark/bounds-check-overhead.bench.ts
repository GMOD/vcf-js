...
function parseGenotypesWithBoundsCheck(
  format: string,
  prerest: string,
  samples: string[],
) {
  ...
  if (format.length === 2) {
    ...
    for (let idx = 0; idx < samplesLen; idx++) {
      if (pos >= prerestLen) break
      if (pos + 3 < prerestLen && (prerest[pos + 3] === ':' || prerest[pos + 3] === '\t')) {
        genotypes[samples[idx]!] = prerest.slice(pos, pos + 3)
        if (prerest[pos + 3] === ':') {
          pos += 4
          while (pos < prerestLen && prerest[pos] !== '\t') pos++
...
// Without bounds check - assume well-formed data
function parseGenotypesNoBoundsCheck(
  format: string,
  prerest: string,
  samples: string[],
) {
...
describe('1000 samples - GT only - bounds check overhead', () => {
  const { samples, genotypeData, format } = generateTestData(1000, 'GT')

  bench('with bounds check', () => {
    parseGenotypesWithBoundsCheck(format, genotypeData, samples)
  })

  bench('without bounds check', () => {
    parseGenotypesNoBoundsCheck(format, genotypeData, samples)
  })
})

describe('5000 samples - GT only - bounds check overhead', () => {
...
describe('5000 samples - GT:DP:GQ - bounds check overhead', () => {
...
describe('5000 samples - DP:GQ:GT - bounds check overhead', () => {
