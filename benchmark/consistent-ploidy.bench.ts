...
// Optimized: assume consistent ploidy after detecting first GT length
function parseGenotypesConsistentPloidy(
  format: string,
  prerest: string,
  samples: string[],
) {
  ...
  if (format === 'GT') {
    // Detect GT length from first sample
    let firstGTLen = 0
    let p = 0
    while (p < prerestLen && prerest.charCodeAt(p) !== TAB) {
      firstGTLen++
      p++
    }

    // If first GT is 3 chars and we have enough samples to make it worth checking
    if (firstGTLen === 3 && samplesLen > 2) {
      // Verify all GTs are 3 chars by checking if total length matches
      const expectedLen = samplesLen * 4 - 1 // (3 chars + tab) * samples - last tab
      if (prerestLen === expectedLen) {
        // Ultra-fast path: all GTs are exactly 3 characters
...
describe('1000 samples - GT only - all same ploidy (diploid)', () => {
  ...
  bench('consistent ploidy optimization', () => {
    parseGenotypesConsistentPloidy(format, genotypeData, samples)
  }, { time: 4000 })
})
