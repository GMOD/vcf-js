...
async function main() {
  if (!process.argv[2]) {
    console.error('Usage: node profile-master.mjs <vcf.gz file>')
    console.error(
      'Example: node profile-master.mjs /home/cdiesh/data/1kg.chr1.subset.vcf.gz',
    )
    process.exit(1)
  }
  ...
  if (!fs.existsSync(masterPath)) {
    ...
    console.error('Run: ./build-both-versions.sh')
    process.exit(1)
  }

  console.log('Profiling MASTER version')
  console.log('========================')
  ...
  const vcfModule = await import(masterPath)
  ...
  return new Promise((resolve, reject) => {
    ...
    rl.on('close', function () {
      ...
      session.post('Profiler.stop', (err, { profile }) => {
        ...
        const outputFile = 'profile-master.cpuprofile'
